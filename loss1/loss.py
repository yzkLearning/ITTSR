from loss1 import discriminator
...
#
#     def forward(self, sr, hr):  # 定义前向传播函数forward，输入为sr和hr
...
class PerceptualLoss(nn.Module):
    ...
    def forward(self, sr_relu5_1, hr_relu5_1):   # 定义前向传播函数forward，输入为sr_relu5_1和hr_relu5_1
        loss = F.mse_loss(sr_relu5_1, hr_relu5_1)  # 计算均方误差损失值
        return loss
...
class TPerceptualLoss(nn.Module):
    def __init__(self, use_S=True, type='l2'):
        ...
        self.use_S = use_S
        self.type = type  # 进行损失计算的类型

    def gram_matrix(self, x):  # 定义Gram矩阵计算函数，输入为x
        b, ch, h, w = x.size()  # 获取x的大小信息
        f = x.view(b, ch, h*w)
        f_T = f.transpose(1, 2)
        G = f.bmm(f_T) / (h * w * ch)
        return G
...
class AdversarialLoss(nn.Module):
    def __init__(self, logger, use_cpu=False, num_gpu=1, gan_type='WGAN_GP', gan_k=1, 
        lr_dis=1e-4, train_crop_size=40):   # 原来是40
